...
descargadearchivo <- function (){
  download.file(url = "https://archive.ics.uci.edu/ml/machine-learning-databases/wine/wine.data", 
                destfile = "D:\\rod_e\\Descargas\\wine.data")
}

importarDatos <- function () {
  #datos = scan(file = "D:\\rod_e\\Descargas\\wine.data", sep = ",")
  #datos = read.table("D:\\rod_e\\Descargas\\wine.data", sep = ",")
  datos <- read.csv(file = "D:\\rod_e\\Descargas\\wine.data", header = FALSE)
...
exportarDatos <- function (datos){
  write.table(datos,"D:\\rod_e\\Descargas\\wineModif.data", sep =",", row.names = FALSE, col.names = FALSE)
